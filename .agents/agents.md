...
| `preflight-report` | Renders each gathered team's markdown fix report from its data + findings files (`ConvertTo-GovernancePreflightReport`), splicing `observations-<code>.md` when present. Offline, deterministic, no AI — the renderer owns every table; the fragment is the only prose anyone else writes (ADR-009). The shipped `/audit-preflight` command orchestrates gather → render → observe → check → publish for every team. |
