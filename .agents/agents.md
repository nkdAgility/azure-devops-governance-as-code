...
| `preflight` | Per incoming team: "what would fail if this team moved in today?" Two steps (ADR-008): **gather** reads the team's PRE-MIGRATION location from `programs/<name>/sources.yaml` plus authored-UPN resolution in the target and writes the facts to `preflight-<code>.data.json`; **analysis** reads that document, projects it into target coordinates and runs the SAME evaluators audit uses (area orphans-to-be with work items per path, tag usage on source work items with disallowed families bundled per pattern, repo naming, member resolvability, unauthored source-team members). `-Offline` re-analyses the last data file with no live calls; `-SkipFresh` gathers only nodes with no data file yet. **Read-only against both orgs.** Findings are objects (`class`, `check`, `subject`, counts, `message`, plus `sources.yaml labels:` per check); writes `<program>-preflight-<CODE>-findings.txt/.json` in `<output>\preflight\<CODE>\`; non-zero exit on findings. Process/type/state compatibility is the migration toolchain's job, not preflight's (ADR-007). |
| `preflight-report` | Renders each gathered team's `-report.md` from its data + findings files (`ConvertTo-GovernancePreflightReport`), splicing that node's `-observations.md` when present. Offline, deterministic, no AI — the renderer owns every table; the fragment is the only prose anyone else writes (ADR-009). The shipped `/audit-preflight` command orchestrates gather → render → observe → check → publish for every team. |
